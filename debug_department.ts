  ...
  try {
    ...
    if (allDepts && allDepts.length > 0) {
      ...
      allDepts.forEach((d, idx) => {
        console.log(
          `  Dept ${idx}: MaPhg=${d.MaPhg}, maphg=${d.maphg}, MAPHG=${d.MAPHG}`,
        );
      });
    ...
    const testDept = await departmentRepository.getDepartmentById(1);
    ...
    const testId =
      Math.max(...(allDepts || []).map((d) => parseInt(d.MaPhg) || 0)) + 1;
