...
const initCronJobs = () => {
    ...
    node_cron_1.default.schedule("0 8 * * *", async () => {
        try {
            ...
            const projects = await db_1.appPool.request().query(`
      ...
      `);
            for (const row of projects.recordset) {
                ...
                const check = await db_1.appPool.request()
                    .input("MaNV", row.MANV)
          ...
          `);
                if (check.recordset.length === 0) {
                    ...
                    const notif = await (0, notificationController_1.createNotification)(row.MANV, `Dự án "${row.TENDA}" sắp đến hạn`, `Dự án của bạn sẽ hết hạn trong ${daysLeft} ngày nữa. Vui lòng kiểm tra tiến độ!`, "project_deadline", `/projects/${row.MADA}`);
                    if (notif)
                        (0, server_1.emitNotification)(row.MANV, notif);
                }
            }
            // 2. Tasks expiring in <= 2 days
            const tasks = await db_1.appPool.request().query(`
      ...
      `);
          ...
          `);
                if (check.recordset.length === 0) {
                    const daysLeft = Math.ceil((new Date(row.NGAYKETTHUC).getTime() - new Date().getTime()) / (1000 * 3600 * 24));
                    const notif = await (0, notificationController_1.createNotification)(row.MANV, `Nhiệm vụ "${row.TENNHIEMVU}" sắp đến hạn`, `Nhiệm vụ của bạn trong dự án sẽ hết hạn trong ${daysLeft} ngày nữa.`, "task_deadline", `/projects/${row.MADA}`);
                    if (notif)
                        (0, server_1.emitNotification)(row.MANV, notif);
                }
            }
