...
async function debugUpdateDepartment() {
    try {
        ...
        const request1 = db_1.appPool.request();
        const existing = await request1
            .input("MaPhg", db_1.sql.Int, 1003)
        ...
        const request2 = db_1.appPool.request();
        request2.output("Status", db_1.sql.Int);
        const result = await request2
            .input("MaPhg", db_1.sql.Int, 1003)
            .input("TenPb", db_1.sql.NVarChar(100), newTenPb)
            .input("MaTruongPhg", db_1.sql.VarChar(10), newMaTruongPhg)
        ...
        const request3 = db_1.appPool.request();
        ...
        if (afterUpdate.recordset && afterUpdate.recordset.length > 0) {
            ...
            console.log("\nSo sánh:");
            console.log(`  TENPB thay đổi: ${currentData.TENPB} → ${updatedData.TENPB}`);
            console.log(`  MaTruongPhg thay đổi: ${currentData.MaTruongPhg} → ${updatedData.MaTruongPhg}`);
        }
    }
    catch (error) {
